...
const SilkPlane = forwardRef<THREE.Mesh, { uniforms: SilkUniforms }>(
  function SilkPlane({ uniforms }, ref) {
    ...
    useLayoutEffect(() => {
      const target = (ref as React.RefObject<THREE.Mesh>)?.current ?? innerRef.current
      if (target) {
        target.scale.set(viewport.width, viewport.height, 1)
      }
    }, [ref, viewport])

    useFrame((_, delta) => {
      const target = (ref as React.RefObject<THREE.Mesh>)?.current ?? innerRef.current
      const mat = target?.material as THREE.ShaderMaterial | undefined
    ...
    return (
      <mesh ref={ref ?? innerRef} material={material}>
